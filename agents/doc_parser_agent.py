...
class DocParserAgent:
    ...
    async def parse(self, file_path: str) -> list[DocumentChunk]:
        ...
        if doc_type == DocType.UNKNOWN:
            ...
            raise ValueError(f"不支持的文档类型: {os.path.splitext(file_path)[1] or '<无扩展名>'}; 支持: {supported}")
    ...
    async def _describe_image_with_llm(self, image: Any) -> str:
        ...
        messages = [
            SystemMessage(content="你是文档分析助手，请详细描述图片中的内容，包括文字、表格、图表信息。"),
